...
#define GPS_DELETE_DATA_MASK 0xFF
...
static int gnss_ctrl(uint32_t ctrl)
{
...
#if defined(CONFIG_GPS_SAMPLE_FORCE_COLD_FIX)
	nrf_gnss_delete_mask_t	delete_mask  = GPS_DELETE_DATA_MASK;
#else
	...
	/* By default, goes by cold start and normal accuracy */
	nrf_gnss_use_case_t use_case = 0;
...
static void print_satellite_stats(nrf_gnss_data_frame_t *pvt_data)
{
	...
	printk("Tracking: %d Using: %d Unhealthy: %d\n", tracked,
							 ...
							 unhealthy);
	printk("Stats:\n");
	for(uint8_t i = 0; i < NRF_GNSS_MAX_SATELLITES ; i++){
		if(pvt_data->pvt.sv[i].signal == 1)
			printk("- SV: %d, Used for Fix: %d, Healthy: %d, CN0*0.1: %d\n",
					pvt_data->pvt.sv[i].sv,
					pvt_data->pvt.sv[i].flags & NRF_GNSS_SV_FLAG_USED_IN_FIX? 1 : 0,
					pvt_data->pvt.sv[i].flags & NRF_GNSS_SV_FLAG_UNHEALTHY? 1 : 0,
					pvt_data->pvt.sv[i].cn0);
	}
	printk("\n");
}
